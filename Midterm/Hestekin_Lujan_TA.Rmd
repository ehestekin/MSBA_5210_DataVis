...
#Add season for temperal analysis (week is too segmented)
full_df <- full_df %>% 
  mutate(season = (week %/% 14) + 1)
...
```{r}
#TODO : would like to grid.arrange these plots but couldn't figure it out when using group_by
full_df %>% 
  group_by(brand) %>% 
...
full_df %>% 
  group_by(flavor) %>% 
  summarize(n_flav = n()) %>% 
  ggplot(mapping = aes(x = flavor, y = n_flav)) +
    geom_bar(stat = 'identity')
