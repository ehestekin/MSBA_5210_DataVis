...
```{r}
# Scatterplots looking at relationships between price and total volume by company
full_df %>% 
  ggplot(data = full_df, mapping = aes(x = (units*volume), y = price)) +
  geom_point() +
  labs(title = "Total volume and price by company") +
  facet_grid(company~promo)

# Scatterplots looking at relationships between price and total volume for GM Toasted brands
full_df %>% 
  filter(flavor == "TOASTED", company == "General Mills") %>% 
  ggplot(mapping = aes(x = (units*volume), y = price)) +
  geom_point() +
  labs(title = "GM toasted flavor, total volume and price by brand") +
  facet_grid(brand~promo)

# Scatterplots looking at relationships between price and total volume for KG Toasted brands
full_df %>% 
  filter(flavor == "TOASTED", company == "Kellogg") %>% 
  ggplot(mapping = aes(x = (units*volume), y = price)) +
  geom_point() +
  labs(title = "KG toasted flavor, total volume and price by brand") +
  facet_grid(brand~promo)
```

- No relationship between total volume and price
  - Expected to see lower prices at higher volumes, given the negative correlation between price and units
  - Price appears to stay within the same ranges across volumes and promo; no increase or decrease apparent

```{r}
# Avg sales by flavor and company, focus on General Mills and Kellogg
full_df %>% 
  filter(flavor != "FRUIT", flavor != "CINNAMON TOAST", flavor != "REGULAR") %>% 
  group_by(company, week, promo, flavor) %>% 
  summarise(sales = mean(total_sales)) %>% 
  ggplot(mapping = aes(x = week, y = sales, group = company, color = company)) + 
  geom_line() +
  labs(title = "Average weekly sales, cocoa and toasted") +
  facet_grid(promo ~ flavor) +
  theme_bw()
```

- GM cocoa avg sales consistently higher with and w/out promo
- GM toasted avg sales higher w/out promo, gap closed w/promo
...
full_df %>% 
  filter(flavor != "CINNAMON TOAST",
         flavor != "FRUIT") %>% 
  group_by(company, promo, brand, flavor) %>% 
  summarise(sales = sum(total_sales)) %>% 
  ggplot(mapping = aes(x = company, y = sales, fill = company)) + 
...
- General Mills loses its lead in toasted flavor with promos but maintains it in cocoa flavor
- What is happening in toasted flavor?

**Closer look at toasted flavor**

```{r}
# Total sales by brand for toasted flavor, promo v no promo
full_df %>% 
  ...
  coord_flip() +
  facet_grid(promo~.) +
  labs(title = "Toasted flavor total sales by brand, no promo v promo")
```

- Without promos, cheerios leads all brands with sales but is more even with other brands with promos
- Total sales for Cheerios seem to decrease with promos, while other brands increase

```{r}
# Total sales by brand for toasted flavor, advs v no ads
full_df %>% 
  filter(flavor == "TOASTED") %>% 
  group_by(brand, company, ad2) %>% 
  ggplot(mapping = aes(x = brand, y = total_sales)) +
  geom_boxplot() +
  coord_flip() +
  facet_grid(ad2~.) +
  labs(title = "Toasted flavor total sales by brand, ads v no ads")
```

- Ads don't seem to make a difference for Cheerios, but make a slight difference for Lucky Charms
- Seem to make more of a difference for Kellogg brands
...
**Closer look at General Mills brands**

```{r}
# Bar plots from above for General Mills Brands only
# Average sales
full_df %>% 
  ...
  summarize(avg_vol = mean(total_volume), avg_sales = mean(total_sales),
            n = n(), sd_vol = sd(total_volume), sd_sales = sd(total_sales),
            ci_vol = z * sd_vol/sqrt(n), ci_sales = z * sd_sales/sqrt(n)) %>% 
  ggplot(mapping = aes(x = brand, y = avg_sales, fill = as.factor(promo))) +
  geom_bar(stat = 'identity', position = 'dodge') +
  coord_flip() +
  labs(title = "Average sales for General Mills brands") +
  geom_errorbar(aes(ymin = avg_sales - ci_sales, ymax = avg_sales + ci_sales),
                width = 0.75, position = position_dodge(0.9))
...
- General Mills:
  - Cheerios avg sales are lower with promos, while every other brand increases or stays the same
  - Cheerios avg vol is about the same with and w/out promos while every other brand increases significantly
  
```{r}
# Bar plots from above for General Mills Brands only
# Average sales
full_df %>% 
  filter(company == "Kellogg") %>% 
  mutate(total_volume = units * volume) %>% 
  group_by(company, brand, promo) %>% 
  summarize(avg_vol = mean(total_volume), avg_sales = mean(total_sales),
            n = n(), sd_vol = sd(total_volume), sd_sales = sd(total_sales),
  ...
  geom_bar(stat = 'identity', position = 'dodge') +
  coord_flip() +
  labs(title = "Average sales for Kellogg's brands") +
  geom_errorbar(aes(ymin = avg_sales - ci_sales, ymax = avg_sales + ci_sales),
                width = 0.75, position = position_dodge(0.9))
```
...
## Test of promo impact to overall General Mills sales

- Ho: GM avg sales w/out promo - GM avg sales w/promo = 0
- Ha: GM avg sales w/out promo - GM avg sales w/promo != 0

Alpha = 0.025

```{r}
# Set df to t for easier subsetting in t tests
t <- full_df

# Test for differences between promo and non-promo sales for General Mills
t.test(t$total_sales[t$company == "General Mills" & t$promo == 0],
       t$total_sales[t$company == "General Mills" & t$promo == 1],
       conf.level = 0.95)
```

- We do not reject the null hypothesis
- There is no statistically significant difference between promo and non-promo sales for General Mills

**Conclusion:** There is no statistical evidence to support that promos make a difference to sales for General Mills overall. However, I will test for differences within each brand. 

## Tests within General Mills brands for differences in sales for promo v no promo

```{r}
# Check distributions of GM brands
full_df %>% 
  filter(company == "General Mills") %>% 
  group_by(brand) %>% 
  ggplot(mapping = aes(x = total_sales)) +
  geom_histogram() +
  facet_grid(brand~promo)
```

- Without promotions, distributions look right-skewed; but will use test of means for practical purposes
...
full_df %>% 
  filter(company == "General Mills") %>% 
  group_by(brand) %>% 
  ggplot(mapping = aes(y = total_sales, x = brand)) +
  geom_boxplot() +
  coord_flip() +
  facet_grid(.~promo)
```

- Can see the medians shift slightly for most brands when there are promos, with major exception being Cheerios


## General Mills brands, promo v no promo

Hypothesis test set-up for all GM brands:

- Ho: Brand avg sales w/ promo - Brand avg sales w/out promo = 0
- Ha: Brand avg sales w/ promo - Brand avg sales w/out promo != 0

Alpha = 0.025

```{r}
# Test for Lucky Charms
(t1 <- t.test(t$total_sales[t$promo == 1 & t$brand == "GENERAL MILLS LUCKY CHARMS"],
             t$total_sales[t$promo == 0 & t$brand == "GENERAL MILLS LUCKY CHARMS"],
             conf.level = 0.95))
```

- reject the null hypothesis
- there is a statistically significant difference between promo and non promo sales for Lucky Charms
- On avg, promo sales are higher than non-promo sales

```{r}
# Test for Kix
(t2 <- t.test(t$total_sales[t$promo == 1 & t$brand == "GENERAL MILLS KIX"],
             t$total_sales[t$promo == 0 & t$brand == "GENERAL MILLS KIX"],
             conf.level = 0.95))
```

- do not reject the null hypothesis
- there is no statistically significant difference between promo and non promo sales for Kix

```{r}
# Test for Cocoa Puffs
(t3 <- t.test(t$total_sales[t$promo == 1 & t$brand == "GENERAL MILLS COCOA PUFFS"],
             t$total_sales[t$promo == 0 & t$brand == "GENERAL MILLS COCOA PUFFS"],
             conf.level = 0.95))
```

- reject the null hypothesis
- there is a statistically significant difference in promo and non-promo sales for Cocoa Puffs
- on avg, promo sales are higher than non-promo sales

```{r}
# Test for Cinnamon Toast
(t4 <- t.test(t$total_sales[t$promo == 1 & t$brand == "GENERAL MILLS CINNAMON TST CR"],
             t$total_sales[t$promo == 0 & t$brand == "GENERAL MILLS CINNAMON TST CR"],
             conf.level = 0.95))
```

- reject the null hypothesis
- there is a statistically significant difference in promo v non promo sales for Cinnamon Toast Crunch
- on avg. promo sales are higher than non-promo sales

```{r}
# Test for Cheerios
(t5 <- t.test(t$total_sales[t$promo == 1 & t$brand == "GENERAL MILLS CHEERIOS"],
             t$total_sales[t$promo == 0 & t$brand == "GENERAL MILLS CHEERIOS"],
             conf.level = 0.95))
```

- reject the null hypothesis
- there is a statistically significant difference between promo and non-promo sales for Cheerios
- on average, promo sales are lower than non-promo sales

## Compare Between General Mills and Kellogg

```{r}
# GM and KG, toasted flavor with promos
t.test(t$total_sales[t$company == "General Mills" & t$promo == 1],
       t$total_sales[t$company == "Kellogg" & t$promo == 1],
       conf.level = 0.95)
```

```{r}
# Difference in mean price between GM and KG when there are promos
t.test(t$price[t$company == "General Mills" & t$promo == 1],
       t$price[t$company == "Kellogg" & t$promo == 1],
       conf.level = 0.95)
```

```{r}
# Difference in mean price between GM and KG for toasted, when there are promos
t.test(t$price[t$company == "General Mills" & t$promo == 1 & t$flavor == "TOASTED"],
       t$price[t$company == "Kellogg" & t$promo == 1 & t$flavor == "TOASTED"],
       conf.level = 0.95)
```

## Test for impact of ads on sales

- Ho: GM avg sales w/ ads - GM avg sales w/out = 0
- Ha: GM avg sales w/ ads - GM avg sales w/out ads != 0

```{r}
# Test for differences between ad v no ad sales for General Mills
t.test(t$total_sales[t$company == "General Mills" & t$ad2 == 1],
       t$total_sales[t$company == "General Mills" & t$ad2 == 0],
       conf.level = 0.95)
```

- We do not reject the null hypothesis
- There is not statistically significant difference between the average sales of General Mills cereals that have ads versus the cereals that don't have ads

**Conclusion:** Ads do not make a consistent difference in overall sales for General Mills cereals. However, about 77% of the time we can expect to see impacts of ads that are either positive or negative for the company overall. 
...
```{r}
# # Relevel brands to have brand with lowest sales as the base
# full_df$brand <- relevel(full_df$brand, ref = "KELLOGGS COCOA KRISPIES")
# # Relevel companies to have Post as the base
# full_df$company <- relevel(full_df$company, ref = "Post")
# 
# # Model with brands only
# mod1 <- lm(total_sales ~ brand, data = full_df)
# summary(mod1)
# # Model with brands, ads, and promos
# mod2 <- lm(total_sales~ brand + promo + ad2, data = full_df)
# summary(mod2)
# 
# # Model with brands, ads, and promos
# mod3 <- lm(price~ volume + promo + ad2 + brand, data = full_df)
# summary(mod3)
```
...
## Finding #1: On average, Cheerios sales decrease with promotions
```{r}
# Set my theme to make visualizations consistent
my_theme <- theme(panel.border = element_blank(),
        ...
        plot.subtitle = element_text(color = "#808080", size = 12, face = "plain"),
...
Based on the hypothesis tests above, we see that not every General Mills brand sees a positive impact to their sales from promotions. Lucky Charms, Cocoa Puffs, and Cinnamon Toast Crunch see positive impacts to their sales that are both practically and statistically significant. Kix sees no statistically significant impact to its average sales, while Cheerios has a large negative impact that is also statistically significant.



```{r}
# Create new object to make visualization easier
fig1 <- full_df %>% 
  filter(company == "General Mills") %>% 
  group_by(brand, promo) %>% 
...
# Lollipop visualization to highlight impacts of promotions to brands
ggplot(data = fig1, mapping = aes(x = reorder(brand, pdifference), y = pdifference)) +
  geom_lollipop(color = "grey70", size = 1.75, point.colour = "#385CB7") +
  my_theme +
  coord_flip() +
  geom_hline(yintercept = 0, color = "gray") +
  labs(title = "Cheerios sales decrease with promotions*",
       subtitle = "Promotional impacts to sales vary across brands",
       x = "",
       y = "Percent Weekly Revenue Change",
       caption = "*For average weekly sales revenue") +
  scale_y_continuous(labels = scales::percent) +
  scale_x_discrete(labels = c("GENERAL MILLS COCOA PUFFS" = "Cocoa Puffs", "GENERAL MILLS LUCKY CHARMS" = "Lucky Charms", "GENERAL MILLS CINNAMON TST CR" = "Cinnamon Toast Crunch", "GENERAL MILLS KIX" = "Kix", "GENERAL MILLS CHEERIOS" = "Cheerios"))

# Save plot for memo
ggsave("figure1.png", width = 7.5, height = 5, units = "in")
...
#   group_by(brand, promo) %>% 
#   summarise(avg_sales = mean(total_sales)) %>% 
#   ggplot(mapping = aes(x = brand, y = avg_sales, group = promo, fill = as.factor(promo))) +
#   geom_col(position = "dodge") +
#   coord_flip()
```

```{r}
# Save df for use in memo
write_csv(full_df, path = 'full_df.csv')
```