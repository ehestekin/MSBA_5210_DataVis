...
```{r}
# Look at flavors where only General Mills and Kellogg compete
grid.arrange(
full_df %>% 
  filter(flavor == "COCOA") %>% 
...
# Look at flavors where only General Mills and Kellogg
full_df %>% 
  filter(flavor == "COCOA") %>% 
  group_by(company, brand, week, promo, flavor) %>% 
  summarise(sales = sum(total_sales)) %>% 
  ggplot(mapping = aes(x = week, y = sales, group = brand, color = company)) + 
  geom_line() +
